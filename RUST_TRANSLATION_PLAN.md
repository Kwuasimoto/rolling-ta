...
- [ ] `ta/error.rs` — TAError enum
- [ ] `ta/types.rs` — Ohlcv, OhlcvSeries, Price, Volume newtypes
- [ ] `ta/state.rs` — IndicatorState enum
- [ ] `ta/config.rs` — Config structs for all indicators
- [ ] `ta/mod.rs` — Indicator trait + re-exports

### Phase 2: Math Layer (Pure Functions)

Translate numba.py functions to Rust. These are stateless helpers.

- [ ] `ta/math/rolling.rs` — Rolling window utilities
- [ ] `ta/math/ema.rs` — EMA multiplier calculation
- [ ] `ta/math/stats.rs` — Mean, variance, std dev

### Phase 3: Simple Indicators

Indicators with single output, no dependencies.

| Priority | Indicator | Complexity | Notes |
|----------|-----------|------------|-------|
| 1 | SMA | Low | Foundation for others |
| 2 | EMA | Low | Foundation for MACD |
| 3 | RSI | Medium | Two EMAs (gain/loss) |
| 4 | TR | Low | Foundation for ATR |
| 5 | ATR | Medium | Composes TR |
| 6 | WMA | Medium | Weighted sum |
| 7 | OBV | Low | Simple accumulator |
...
/// Test Rust output matches Python rolling-ta output
#[test]
fn test_rsi_matches_python() {
    // Known test data
    let close = include!("fixtures/btc_close_1000.rs");  // Generated from Python
    let expected = include!("fixtures/btc_rsi_14.rs");   // From Python rolling-ta

    let (output, _) = rsi_batch(&close, 14);

    for (i, (rust, python)) in output.iter().zip(expected.iter()).enumerate() {
        if !rust.is_nan() && !python.is_nan() {
            assert_relative_eq!(rust, python, epsilon = 1e-6,
                "Mismatch at index {}: Rust={}, Python={}", i, rust, python);
        }
    }
}
```
