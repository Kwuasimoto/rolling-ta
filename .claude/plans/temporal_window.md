...
2. Why `next(&[Ohlcv])` replaces `update(&Ohlcv)`:
   ...
   | Push logic | Indicator manages | Runner manages |
...
**Key insight:** Stateful indicators (RSI, EMA, ATR) only need the latest candle from the snapshot. The runner can optimize by passing `&candles[candles.len()-1..]` for these.

### Phase 4: Indicator Runner with Rayon

**File: `src/ta/runner.rs` (new)**
...
// Create runner
let mut runner = IndicatorManager::new(Arc::clone(&window));

// Register indicators
runner.register(Box::new(SMA::new(SMAConfig::new(14))));
runner.register(Box::new(EMA::new(EMAConfig::new(14))));
runner.register(Box::new(RSI::new(RSIConfig::new(14))));
...
2. **History tracking**: Current indicators store `history: Vec<f64>`. Should this move to IndicatorManager?
   ...
   - Option B: Runner stores `HashMap<&str, Vec<f64>>` of all indicator histories
...
4. **Indicator identification**: How to identify indicators in runner results?
