...
use rolling_ta::momentum::{RSI, RSIConfig};
use rolling_ta::prelude::*;

#[test]
fn rsi_modes_equivalent() {
    ...
    let mut batch = RSI::new(RSIConfig::new(14));
    batch.calc(&data).unwrap();

    let comparisons = compare_values("RSI batch vs reference", batch.history(), expected, EPSILON);
    println!(
    ...
    let mut stream = RSI::new(RSIConfig::new(14));
    for i in 0..data.len() {
    ...
    let half = data.len() / 2;
    let mut hybrid = RSI::new(RSIConfig::new(14));
    ...
    let epsilon = 1e-6;
    assert_histories_equal("RSI batch vs stream", batch.history(), stream.history(), epsilon);
    assert_histories_equal("RSI batch vs hybrid", batch.history(), hybrid.history(), epsilon);
}
