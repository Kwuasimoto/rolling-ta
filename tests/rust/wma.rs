...
fn wma_batch_vs_reference() {
    ...
    let comparisons = compare_values("WMA batch vs reference", wma.history(), expected, EPSILON);
    println!(
...
#[test]
fn wma_streaming_next_vs_batch() {
    // btc-wma.xlsx: timestamp(0), close(1), weights(2), weighted_sum(3), wma(4)
    let cols = read_xlsx_by_position("resources/data/btc-wma.xlsx", &[1, 4]);
    let closes = &cols[0];
    let candles = build_candles_from_closes(closes);
    ...
    let epsilon = 1e-6;
    assert_histories_equal("WMA batch vs stream", &batch_computed, stream_computed, epsilon);
}

#[test]
fn wma_next_with_fixed_window() {
    ...
    let period = 14;
    let mut wma = WMA::new(WMAConfig::new(period));

    // Feed snapshots of exactly `period` candles (like SharedWindow.snapshot_last(14))
    // WMA only needs the last N candles, so fixed window works
    for i in period..candles.len() {
        let snapshot = &candles[i - period + 1..=i];
        let result = wma.next(snapshot);
        ...
        if !rust_val.is_nan() && !true_val.is_nan() {
            ...
            assert!(
                diff < EPSILON,
                "WMA mismatch at index {}: Rust={:.6}, Expected={:.6}, diff={:.6}",
