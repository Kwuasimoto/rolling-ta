...
fn hma_modes_equivalent() {
    ...
    let mut batch = HMA::new(HMAConfig::new(14));
    batch.calc(&data).unwrap();

    let comparisons = compare_values("HMA batch vs reference", batch.history(), expected, EPSILON);
    println!(
    ...
    let mut stream = HMA::new(HMAConfig::new(14));
    for i in 0..data.len() {
    ...
    let half = data.len() / 2;
    let mut hybrid = HMA::new(HMAConfig::new(14));
    ...
    let epsilon = 1e-6;
    assert_histories_equal("HMA batch vs stream", batch.history(), stream.history(), epsilon);
    assert_histories_equal("HMA batch vs hybrid", batch.history(), hybrid.history(), epsilon);
}