...
def test_bb_ma(bb_df: pd.DataFrame, evaluate: Eval):
    ...
    rolling = BollingerBands(data=bb_df, init=True).to_numpy(dtype=np.float64).round(6)
...
def test_bb_upper(bb_df: pd.DataFrame, evaluate: Eval):
    ...
    rolling = (
        BollingerBands(data=bb_df, init=True)
        .to_numpy(get="upper", dtype=np.float64)
        .round(6)
    )
...
def test_bb_lower(bb_df: pd.DataFrame, evaluate: Eval):
    ...
    rolling = (
        BollingerBands(data=bb_df, init=True)
        .to_numpy(get="lower", dtype=np.float64)
        .round(6)
    )
